...
playoff_url = "https://stats.nba.com/stats/leaguedashplayerstats?College=&Conference=&Country=&DateFrom=&DateTo=&Division=&DraftPick=&DraftYear=&GameScope=&GameSegment=&Height=&LastNGames=0&LeagueID=00&Location=&MeasureType=Base&Month=0&OpponentTeamID=0&Outcome=&PORound=0&PaceAdjust=N&PerMode=PerGame&Period=0&PlayerExperience=&PlayerPosition=&PlusMinus=N&Rank=N&Season=2020-21&SeasonSegment=&SeasonType=Playoffs&ShotClockRange=&StarterBench=&TeamID=0&TwoWay=0&VsConference=&VsDivision=&Weight=" 
...
for ii in range(1996,2022):
    ...
    if not exists("/home/gene/Documents/DataMiningII/Project/getNBAcom/nonAdvancedCSV/"+ yearString +"_regular.csv"):
        #vpn = vpnList[count]
        ...
        frame.to_csv("/home/gene/Documents/DataMiningII/Project/getNBAcom/nonAdvancedCSV/"+ yearString +"_regular.csv",sep=",",header=frame.columns)

    # Next get playoffs
    if not exists("/home/gene/Documents/DataMiningII/Project/getNBAcom/nonAdvancedCSV/"+ yearString +"_playoff.csv"):
        currURL = playoff_url.replace("2020-21",yearString)
        response = requests.get(currURL, headers=header)
        response_json = response.json()
        frame = pd.DataFrame(response_json['resultSets'][0]['rowSet'])
        frame.columns = response_json['resultSets'][0]['headers']
        frame.to_csv("/home/gene/Documents/DataMiningII/Project/getNBAcom/nonAdvancedCSV/"+ yearString +"_playoff.csv",sep=",",header=frame.columns)
