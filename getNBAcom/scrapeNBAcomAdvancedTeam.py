...
for ii in range(1996,2022):
    ...
    # Next get playoffs
    if not exists("/home/gene/Documents/DataMiningII/Project/getNBAcom/teamAdvancedCSV/"+ yearString +"_playoff.csv"):
        currURL = playoff_url.replace("2019-20",yearString)
        response = requests.get(currURL, headers=header)
        response_json = response.json()
        frame = pd.DataFrame(response_json['resultSets'][0]['rowSet'])
        frame.columns = response_json['resultSets'][0]['headers']
        frame.to_csv("/home/gene/Documents/DataMiningII/Project/getNBAcom/teamAdvancedCSV/"+ yearString +"_playoff.csv",sep=",",header=frame.columns)
